    ...
    with sqlite_lock:
        with sqlite3.connect(brickdir / 'predictions.sqlite') as conn:
            ...
            for inchi, property_token in inchi_tok_pairs:
                cursor = conn.execute("""SELECT inchi, property_token, positive_prediction FROM predictions 
...
def get_predictions_with_sqlite_cache(inchi_tok_pairs) -> dict:
    ...
    inchi_tok_predictions = {p[1]: p[2] for p in predictions}
    remaining_inchi_tok_pairs = [(i, t) for i, t in inchi_tok_pairs if t not in inchi_tok_predictions]
    for inchi, tok in tqdm(remaining_inchi_tok_pairs):
        result = chemprop.get_chemprop_prediction(inchi=inchi, property_token=tok)
        add_predictions([(inchi, tok, result['positive_prediction'])], sqlite_lock)
        inchi_tok_predictions[tok] = result['positive_prediction']
    return inchi_tok_predictions
